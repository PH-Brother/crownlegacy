...
serve(async (req) => {
  ...
  try {
    ...
    if (!LOVABLE_API_KEY) {
      ...
      return new Response(JSON.stringify({ error: "Service configuration error" }), { status: 500, headers: jsonHeaders });
    }

    console.log(JSON.stringify({ event: "ai_request", user_id: userId, tipo, timestamp: new Date().toISOString() }));
    ...
    if (!response.ok) {
      ...
      if (status === 429) {
        return new Response(JSON.stringify({ error: "Limite de requisições excedido. Tente novamente em alguns minutos." }), { status: 429, headers: jsonHeaders });
      }
      if (status === 402) {
        return new Response(JSON.stringify({ error: "Créditos insuficientes." }), { status: 402, headers: jsonHeaders });
      }
      return new Response(JSON.stringify({ error: "Erro ao processar análise" }), { status: 502, headers: jsonHeaders });
    }

    const data = await response.json();
    const content = data.choices?.[0]?.message?.content || "Não foi possível gerar a análise.";

    return new Response(JSON.stringify({ resultado: content }), { headers: jsonHeaders });
  } catch (e) {
