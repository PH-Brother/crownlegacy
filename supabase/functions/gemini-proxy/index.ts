...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!key) return new Response(JSON.stringify({error:"sem chave"}), {status:500,headers:{...cors,"Content-Type":"application/json"}})

    const b = await req.json()
    if (!b.base64Data || !b.mimeType) return new Response(JSON.stringify({error:"parametros ausentes"}), {status:400,headers:{...cors,"Content-Type":"application/json"}})
    ...
    if (!r.ok) return new Response(JSON.stringify({error:"gemini "+r.status,details:t.substring(0,300)}), {status:502,headers:{...cors,"Content-Type":"application/json"}})

    const d = JSON.parse(t)
    const txt = d.candidates[0].content.parts[0].text
    if (!txt) return new Response(JSON.stringify({error:"sem conteudo"}), {status:500,headers:{...cors,"Content-Type":"application/json"}})

    const clean = txt.split("```json").join("").split("```").join("").trim()

    let res
    try { res = JSON.parse(clean) }
    catch(e) { return new Response(JSON.stringify({error:"json invalido",raw:clean.substring(0,200)}), {status:422,headers:{...cors,"Content-Type":"application/json"}}) }

    console.log("transacoes:", res.transacoes ? res.transacoes.length : 0)
    return new Response(JSON.stringify({resultado:res}), {headers:{...cors,"Content-Type":"application/json"}})
