...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.base64Data && body.mimeType) {
      ...
      requestBody = {
        contents: [{ parts: [{ text: "Voce e um consultor financeiro cristao. Analise os dados e retorne insights em markdown com emojis. Dados: " + dados }] }],
        generationConfig: {
          temperature: 0.7,
          maxOutputTokens: 8192
        }
      ...
      requestBody = {
        contents: [{ parts: [{ text: "Gere uma reflexao diaria sobre financas e fe crista. Inclua versiculo biblico, aplicacao pratica e dica financeira. Responda em markdown com emojis." }] }],
        generationConfig: {
          temperature: 0.7,
          maxOutputTokens: 8192
        }
    ...
    if (finishReason === "MAX_TOKENS") {
      return new Response(
        JSON.stringify({ error: "Documento muito grande. Tente um arquivo menor." }),
    ...
    if (!part) {
      return new Response(
        JSON.stringify({ error: "Sem conteudo retornado pelo Gemini" }),
        { status: 500, headers: { ...cors, "Content-Type": "application/json" } }
      )
    }
    ...
    if (body.base64Data && body.mimeType) {
      ...
      if (!resultado) {
        return new Response(
          JSON.stringify({ error: "Resultado vazio apos parse" }),
          { status: 422, headers: { ...cors, "Content-Type": "application/json" } }
        )
      }
    ...
    if (!texto) {
      return new Response(
        JSON.stringify({ error: "Texto vazio retornado" }),
        { status: 500, headers: { ...cors, "Content-Type": "application/json" } }
      )
    }
