...
export default function Goals() {
  ...
  const openEdit = (id: string) => {
    ...
    setCurrentValue(String(g.current_value));
    setTargetDate(g.target_date);
    setLinkedAsset(g.linked_asset_id || "");
    setPriority(String(g.priority));
    setVerse(g.motivational_verse || "");
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      linked_asset_id: linkedAsset || null,
      priority: parseInt(priority),
      status: editGoal?.status || "active",
      motivational_verse: verse || null,
      family_id: null,
    };
    if (editGoal) {
        ...
        <div className="mx-auto max-w-[520px] px-4 py-4 pb-24 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-bold text-foreground">🎯 Metas de Patrimônio</h1>
            </div>
            <Button size="sm" className="gradient-gold text-primary-foreground" onClick={openCreate}>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              {tab === "active" && (
                <Button size="sm" className="mt-3 gradient-gold text-primary-foreground" onClick={openCreate}>
                  Criar primeira meta
                </Button>
              )}
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <Select value={priority} onValueChange={setPriority}>
                    ...
                    <SelectContent>
                      {GOAL_PRIORITIES.map((p) => <SelectItem key={p.value} value={String(p.value)}>{p.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                ...
                <Button onClick={handleSave} disabled={saving || !canSave} className="w-full gradient-gold text-primary-foreground">
